...
class Portfolio(db.Model):
    ...
    def to_dict(self):
        """
        Serialize the Portfolio model with normalized URLs
        for images, media, and FAQs.
        """
        def normalize(url):
            """Convert relative/static URLs to full URLs."""
        ...
        return {
            ...
            "icon": self.icon,
            "images": [normalize(img.url) for img in self.images],
            "media": [
                {
                    "type": m.type,
                    "url": normalize(m.url),
